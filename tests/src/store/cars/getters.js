...
export function carIsBooked(car, day, booked = []) {
  return (
    booked.findIndex(item => item.id === car.id && item.date === day) !== -1
  );
}
...
export default {
  ...
  carItems(state) {
    ...
    if (speedValue || runValue) {
      if (speedValue) {
        const { max = 1000, min } = state.maxSpeedItems.find(
          item => item.id === speedValue
        );
        items = state.cars.filter(
          item => item.maxSpeed >= min && item.maxSpeed <= max
        );
      }
      if (runValue) {
        const { max = 1000, min } = state.runItems.find(
          item => item.id === runValue
        );
        items = items.filter(
          item => item.currentRun >= min && item.currentRun <= max
        );
      }
    }
  ...
  bookedCars(state) {
    const sortedBooking = sortBy(state.bookedCars, [
      function(book) {
